...
Если дан график функции y=ƒ(x), то условие «точка лежит ниже графика» записывается y<ƒ(x), а «точка лежит выше графика»: y<ƒ(x).
...
Область A: точка лежит под параболой, над диагональной прямой и слева от оси ординат. Таким образом, условие для нее будет y < 2 — x * x && y > x && x < 0.

Область B: точка лежит под параболой, над осью абсцисс и справа от оси ординат. Таким образом, условие для нее будет y < 2 — x * x && y > 0 && x >= 0.
